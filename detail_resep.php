<?php
// Aktifkan debug sementara (hapus di produksi)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['tambah_ke_belanjaan'])) {
    ...
    $porsi_default = (int) $resep['porsi_default'];

    if ($porsi_default > 0) { // Menghindari division by zero
...
$res_base = $conn->query("SELECT nama_bahan as nama, jumlah_beli as jumlah, satuan_beli as satuan, harga_beli as harga 
                          FROM base_ingredients WHERE user_id = $user_id");
if ($res_base) {
    ...
    <div>
        ...
        <main class="max-w-4xl mx-auto p-6">
            ...
            <div class="flex items-center gap-4 mb-6">
                ...
                <form action="hapus_resep.php" method="POST" onsubmit="return confirm('Anda yakin ingin menghapus resep ini?');">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 items-end">
                    <div>
                        ...
                        <div class="flex items-center mt-1">
                            <input type="number" id="margin-input" value="100" class="w-full text-lg bg-transparent border-b-2 border-gray-400 focus:border-primary focus:ring-0 p-1">
            ...
            <section>
                ...
                <div class="space-y-6">
                    ...
                    foreach ($langkah_langkah as $index => $langkah):
                        if (empty(trim($langkah))) continue;
                    ?>
                    <div class="flex items-start gap-4">
                        <div class="flex-shrink-0 w-8 h-8 rounded-full bg-primary flex items-center justify-center text-white font-bold text-sm"><?php echo $index + 1; ?></div>
                        <p class="text-content-light dark:text-content-dark text-lg mt-1 leading-relaxed"><?php echo htmlspecialchars(trim($langkah)); ?></p>
                    </div>
                    <?php endforeach; ?>
                </div>
            </section>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const basePortion = <?php echo (int)($resep['porsi_default'] > 0 ? $resep['porsi_default'] : 1); ?>;
    let totalModal = 0;
    let marginType = '%'; 
